...
% (Why is Judge Scores linear in popularity? We can consider log/exponential mapping if needed.)
\subsubsection{Improved Weight Mapping: From Absolute Scores to Relative Ranks}
We introduce a non-linear mapping. Instead of directly using $J_{ij}$, we compute the weekly relative rank score:
\begin{equation}
	Score'_{ij} = \frac{(N_j + 1) - \text{Rank}_{ij}}{N_j}
\end{equation}
where $N_j$ is the number of contestants remaining in week $j$. This improvement removes inconsistent judging scales and better models audience voting driven by ``who performed best'' rather than absolute points.
...
\begin{table}[htbp]
  ...
  \caption{Changes in final rank and survival weeks for controversial samples under two rules.}
	\begin{tabular}{cclcc}
\toprule
	\multicolumn{1}{l}{Season} & \multicolumn{1}{l}{Week} & Sample & \multicolumn{1}{l}{忖P} & \multicolumn{1}{l}{忖W} \\
\midrule
	2     & 5     & Jerry Rice & -1    & -1 \\
	4     & 6     & Billy Ray & -2    & -3 \\
...
\textit{Open Journal of Social Sciences}, vol. 8, no. 9, pp. 310每326, 2020.

\bibitem{Shapley1954} 
L. S. Shapley \& M. Shubik. 
``A Method for Evaluating the Distribution of Power in a Committee System.'' 
\textit{American Political Science Review}, vol. 48, no. 3, pp. 787每792, 1954.
...
In \textit{Voting Theory}, \texttt{https://en.wikipedia.org/wiki/Probabilistic_voting}.
